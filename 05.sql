...
SELECT COUNT(*) INTO C 
FROM ALL_TRIGGERS 
  WHERE TRIGGER_NAME = 'TRG_Spec_SPECIALIZACE_ID'; 
  IF (C > 0) THEN 
    EXECUTE IMMEDIATE 'DROP TRIGGER "TRG_Spec_SPECIALIZACE_ID"'; 
END IF; 
END; 
/
...
SELECT COUNT(*) INTO C 
FROM ALL_SEQUENCES 
  WHERE SEQUENCE_NAME = 'SEQ_Spec_SPECIALIZACE_ID'; 
  IF (C > 0) THEN 
    EXECUTE IMMEDIATE 'DROP SEQUENCE "SEQ_Spec_SPECIALIZACE_ID"'; 
END IF; 
END; 
/
...
CREATE SEQUENCE "SEQ_Spec_SPECIALIZACE_ID" 
	INCREMENT BY 1 
	START WITH 1 
	NOMAXVALUE 
	MINVALUE  1 
...
CREATE OR REPLACE TRIGGER "TRG_Spec_SPECIALIZACE_ID" 
	BEFORE INSERT 
	ON "Specializace" 
	FOR EACH ROW 
	BEGIN 
		SELECT "SEQ_Spec_SPECIALIZACE_ID".NEXTVAL 
